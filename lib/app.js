...
const init = (config) => {
  ...
  const maintenance = (cb) => {
    app.use((req, res, next) => {
      ...
      const maintenanceOverrideHeader = config.osseus_server.maintenance_override_header

      // allowed override header
      if (maintenanceOverrideHeader && req.header[maintenanceOverrideHeader]) return next()
