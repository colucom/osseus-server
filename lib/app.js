...
const init = function (config) {
  ...
  const corsHandling = function (cb) {
  ...
  const statics = function (cb) {
    const cwd = process.cwd()

    const engineType = config.get('STATIC_ENGINE_TYPE')
    const viewsFolder = config.get('VIEWS_FOLDER') ? path.join(cwd, config.get('VIEWS_FOLDER')) : ''
    const staticFolder = config.get('STATIC_FOLDER') ? path.join(cwd, config.get('STATIC_FOLDER')) : ''
